...
public class CompatWarnState {
    ...
    private CompatWarnState(MinecraftClient client, HashSet<String> alreadyWarned) {
            ...
            try (CloseableHttpClient http = HttpClients.createDefault()) {
                ...
                try (CloseableHttpResponse response = http.execute(request)) {
                    ...
                    for (JsonElement modInfoElement : element.getAsJsonArray()) {
                        availableCompatPacks.add(ForgeSeasons.GSON.fromJson(modInfoElement, ModInfo.class));
                    }
                }
            }
        }catch (Exception e) {
            ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to request compatibility mods list.", e);
        }
        ...
        if(handler != null) {
            ...
            ForgeSeasons.SEEDS_MAP.forEach((item, block) -> {
    ...
    private void saveState() {
        ...
        try {
            ...
            ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to save season compat warn state.", e);
        }
    }
    ...
    public static CompatWarnState loadState(MinecraftClient client) {
        ...
        NbtCompound nbt;
        try {
            nbt = NbtIo.readCompressed(compatWarnFile.toPath(), NbtSizeTracker.of(0x6400000L));
        } catch (Exception e) {
            nbt = new NbtCompound();
        }
