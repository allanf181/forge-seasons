...
public class FoliageSeasonColors implements SimpleSynchronousResourceReloadListener {
    ...
    private static final Identifier SPRING_FOLIAGE_COLORMAP = ForgeSeasons.identifier("textures/colormap/spring_foliage.png");
    private static final Identifier SUMMER_FOLIAGE_COLORMAP = ForgeSeasons.identifier("textures/colormap/summer_foliage.png");
    private static final Identifier FALL_FOLIAGE_COLORMAP = ForgeSeasons.identifier("textures/colormap/fall_foliage.png");
    private static final Identifier WINTER_FOLIAGE_COLORMAP = ForgeSeasons.identifier("textures/colormap/winter_foliage.png");
    ...
    public Identifier getFabricId() {
        return ForgeSeasons.identifier("foliage_season_colors");
    }
    ...
    public void reload(ResourceManager manager) {
        try{
            Resource spruceFoliage = manager.getResource(ForgeSeasons.identifier("hardcoded/foliage/spruce.json")).orElseThrow();
            ...
            Resource birchFoliage = manager.getResource(ForgeSeasons.identifier("hardcoded/foliage/birch.json")).orElseThrow();
            ...
            Resource defaultFoliage = manager.getResource(ForgeSeasons.identifier("hardcoded/foliage/default.json")).orElseThrow();
            ...
            ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to load hardcoded foliage colors", e);
        }
        ...
        manager.findResources("seasons/foliage", id -> id.getPath().endsWith(".json")).forEach((id, resource) -> {
            ...
            try {
                ...
                ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to load biome foliage colors for: "+biomeIdentifier, e);
            }
        });
        if(!foliageColorMap.isEmpty()) {
            ForgeSeasons.LOGGER.info("["+MOD_NAME+"] Successfully loaded "+foliageColorMap.size()+" custom foliage colors.");
        }
        try {
            ...
            ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to load foliage color texture", e);
        }
    }
}
