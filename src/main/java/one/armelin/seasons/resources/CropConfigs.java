...
public class CropConfigs implements SimpleSynchronousResourceReloadListener {
    ...
    public Identifier getFabricId() {
        return ForgeSeasons.identifier("crop_configs");
    }

    @Override
    public void reload(ResourceManager manager) {
        try {
            Resource resource = manager.getResource(ForgeSeasons.identifier("hardcoded/crop/default.json")).orElseThrow();
        ...
        manager.findResources("seasons/crop", id -> id.getPath().endsWith(".json")).forEach((id, resource) -> {
            ...
            try {
                ...
                ForgeSeasons.LOGGER.error("[" + MOD_NAME + "] Failed to load crop config for: " + cropIdentifier, e);
            }
        });
        
        if (!cropConfigMap.isEmpty()) {
            ForgeSeasons.LOGGER.info("[" + MOD_NAME + "] Successfully loaded {} custom crop configs.", cropConfigMap.size());
        }
    }
}
