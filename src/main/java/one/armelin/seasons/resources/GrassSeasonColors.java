...
public class GrassSeasonColors implements SimpleSynchronousResourceReloadListener {
    ...
    private static final Identifier SPRING_GRASS_COLORMAP = ForgeSeasons.identifier("textures/colormap/spring_grass.png");
    private static final Identifier SUMMER_GRASS_COLORMAP = ForgeSeasons.identifier("textures/colormap/summer_grass.png");
    private static final Identifier FALL_GRASS_COLORMAP = ForgeSeasons.identifier("textures/colormap/fall_grass.png");
    private static final Identifier WINTER_GRASS_COLORMAP = ForgeSeasons.identifier("textures/colormap/winter_grass.png");
    ...
    public Identifier getFabricId() {
        return ForgeSeasons.identifier("grass_season_colors");
    }
    ...
    public void reload(ResourceManager manager) {
        try{
            Resource swampGrass1 = manager.getResource(ForgeSeasons.identifier("hardcoded/grass/swamp1.json")).orElseThrow();
            ...
            Resource swampGrass2 = manager.getResource(ForgeSeasons.identifier("hardcoded/grass/swamp2.json")).orElseThrow();
            ...
            ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to load hardcoded grass colors", e);
        }
        grassColorMap.clear();
        manager.findResources("seasons/grass", id -> id.getPath().endsWith(".json")).forEach((id, resource) -> {
            ...
            try {
                ...
                ForgeSeasons.LOGGER.error("["+MOD_NAME+"] Failed to load biome grass colors for: "+biomeIdentifier, e);
            }
        });
        if(!grassColorMap.isEmpty()) {
            ForgeSeasons.LOGGER.info("["+MOD_NAME+"] Successfully loaded "+grassColorMap.size()+" custom grass colors.");
        }
        try {
